...
class Session(BaseModel):
    ...
    @classmethod
    def remove_date_hypens(cls, v):
        return v.replace('-', '')

    @field_validator('project')
    @classmethod
    def is_project_exists(cls, v, info: ValidationInfo):
        group = info.data.get('group')
        if group and v in list(map(lambda x: x.name, settings.groups[group].projects)):
            return v
        raise ProjectDoesNotExistError()

    @field_validator('gainReference')
    @classmethod
    def is_gain_empty(cls, v):
        if v == '':
            return None
        return v

    @field_validator('slitWidth')
    @classmethod
    def is_slit_width_null(cls, v):
        if v == 0:
            return None
        return v
...
def save_session(session: Session, directory: Path = settings.env.sessionsDirectory):
    ...
    with open(file, 'w') as f:
        f.write(yaml.dump(session.model_dump()))
...
def load_session_from_file(session: str) -> Tuple[str, Session]:
    ...
    session = Session.model_validate(yaml_file)
    return file, session
